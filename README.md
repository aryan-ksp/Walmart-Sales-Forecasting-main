# Walmart Store Sales Forecasting - Aryan Kashyap
Objective:
The goal is to forecast store sales using historical markdown data.

Overview:
You have access to historical sales records from 45 Walmart stores across various regions. Each store consists of multiple departments, and the task is to predict department-wise sales for each store.

Files Provided:
Final_outputs.xlsx – Contains the predicted sales.
Jupyter Notebook (ipynb) – The necessary code for analysis and modeling.
Dataset & Additional Resources – Includes sales data and relevant materials.


Business Context:  
The objective is predicting store sales using historical markdown data.  One challenge of  modelling retail data is the need to make decisions based on limited 
